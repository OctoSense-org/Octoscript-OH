...
pub fn apply_scroll() {
    ...
    let Some(y) = (unsafe {
        Node::get_f32(
            scroll as octoscript_oh_arkui::arkui::NodeHandle,
            attr::scroll_offset(),
            1,
        )
    }) else {
    ...
    if img != 0 {
        ...
        unsafe {
            Node::set_f32_attr_raw(
                img as octoscript_oh_arkui::arkui::NodeHandle,
                attr::opacity(),
    ...
    if title != 0 {
        ...
        unsafe {
            Node::set_f32v_raw(
                title as octoscript_oh_arkui::arkui::NodeHandle,
                attr::translate(),
                &[0.0, slide, 0.0],
            );
            Node::set_f32_attr_raw(
...
fn editorial(wonder: &Wonder, index: usize, w: f32, h: f32) -> Option<Node> {
    ...
    s = s.on_event(octoscript_oh_arkui::arkui::event::did_scroll(), SCROLL_TICK);
...
fn video(wonder: &Wonder, e: &super::editorial_data::Editorial, w: f32) -> Option<Node> {
    ...
    c = c.child(frame.on_event(octoscript_oh_arkui::arkui::event::click(), VIDEO_TAP));
    c = c.child(
...
fn slide_wall(sel: usize) {
    ...
    let places: Vec<(usize, f32, f32)> = wall
        ...
        .collect();
    unsafe {
        octoscript_oh_arkui::arkui::animate(
            anchor as octoscript_oh_arkui::arkui::NodeHandle,
            120,
            octoscript_oh_arkui::arkui::CURVE_EASE_OUT,
            move || {
                for (c, x, y) in places {
                    unsafe {
                        Node::set_f32v_raw(
                            c as octoscript_oh_arkui::arkui::NodeHandle,
                            attr::position(),
                            &[x, y],
                        )
                    };
...
fn photos(wonder: &Wonder, w: f32, h: f32) -> Option<Node> {
    ...
    let mut root = stack(w, h, wonder.bg)?
        ...
        .on_event(octoscript_oh_arkui::arkui::event::touch(), PHOTO_SWIPE);
            ...
            move || {
                for (bg, frame, pic, title, dot, k) in moves {
                    ...
                    unsafe {
                        ...
                        Node::set_f32_attr_raw(
                            bg as octoscript_oh_arkui::arkui::NodeHandle,
                            attr::opacity(),
                            on,
                        );
                        Node::set_f32_attr_raw(
                        ...
                        Node::set_f32_attr_raw(
                            dot as octoscript_oh_arkui::arkui::NodeHandle,
                            attr::width(),
...
unsafe fn set_box(raw: usize, x: f32, y: f32, w: f32, h: f32, opacity: f32) {
    let n = raw as octoscript_oh_arkui::arkui::NodeHandle;
    unsafe {
