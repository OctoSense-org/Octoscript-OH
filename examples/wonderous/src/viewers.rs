...
pub fn video_viewer(wonder: usize, w: f32, h: f32) -> Option<Node> {
    ...
    // the same film.
    octoscript_oh_arkui::web_declare(
...
pub fn map_block(wonder: usize, w: f32) -> Option<Node> {
    ...
    for dy in -(rows / 2 + 1)..=(rows / 2 + 1) {
        for dx in -(cols / 2 + 1)..=(cols / 2 + 1) {
            ...
            if let Some(n) = Node::new(octoscript_oh_arkui::arkui::ty::image()) {
                root = root.child(
                    n.width(t)
                        .height(t)
                        .string_attr(
    ...
    Some(root.on_event(octoscript_oh_arkui::arkui::event::click(), MAP_TAP))
}
