...
///
/// Declared here rather than beside its only user in `octoscript-oh`, so that
...
) -> Option<octoscript_oh_arkui::arkui::Node> {
...
pub fn hits_swipe(
    ...
    use octoscript_oh_arkui::arkui::attr;
    use octoscript_oh_arkui::ui::{col, row, spacer, tap_col};
    ...
    for band in bands {
        ...
        for (tx, _, tw, th, id) in band {
            ...
            if let Some(base) = swipe {
                t = t.on_event(octoscript_oh_arkui::arkui::event::touch(), base);
            }
            line = line.child(t);
            x = tx + tw;
        }
