...
use octoscript_oh_arkui::arkui::{attr, Node};
...
pub fn read_range() -> bool {
    ...
    let get = |n: usize| unsafe {
        Node::get_f32(
            n as octoscript_oh_arkui::arkui::NodeHandle,
            attr::slider_value(),
            0,
        )
    };
...
fn apply_filter() {
    ...
    for (i, &(tile, label)) in grid.tiles.iter().enumerate() {
        ...
        unsafe {
            Node::set_i32_raw(
                tile as octoscript_oh_arkui::arkui::NodeHandle,
                attr::visibility(),
                vis,
            );
            Node::set_i32_raw(
                label as octoscript_oh_arkui::arkui::NodeHandle,
                attr::visibility(),
                vis,
            );
        }
    ...
    if grid.count != 0 {
        unsafe {
            Node::set_string_raw(
                grid.count as octoscript_oh_arkui::arkui::NodeHandle,
                attr::text_content(),
...
pub fn read_typed() -> bool {
    ...
    let Some(v) = (unsafe {
        octoscript_oh_arkui::arkui::Node::get_string(
            node as octoscript_oh_arkui::arkui::NodeHandle,
            attr::input_text(),
        )
    }) else {
        return false;
...
pub fn build(index: usize, chip: Option<usize>, w: f32, h: f32) -> Option<Node> {
    ...
    let field = Node::new(octoscript_oh_arkui::arkui::ty::input())?
        .width(w - 48.0)
        .height(46.0)
        .bg(0x1AF8ECE5)
        .radius(23.0)
        ...
        .on_event(octoscript_oh_arkui::arkui::event::input_change(), SEARCH_TYPED);
    if let Ok(mut g) = FIELD.lock() {
    ...
    for i in 0..GRID_MAX {
        ...
        let tile = Node::new(octoscript_oh_arkui::arkui::ty::image())?
            .width(gw)
            .height(gw)
            .radius(6.0)
            .bg(0x14F8ECE5)
    ...
    if open {
            ...
            .enumerate()
        {
            ...
            let sl = Node::new(octoscript_oh_arkui::arkui::ty::slider())?
                .width(panel_w - 150.0)
                .height(32.0)
                ...
                .on_event(octoscript_oh_arkui::arkui::event::slider_change(), *id);
            if k == 0 {
