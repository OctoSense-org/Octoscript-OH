# Weather card — generation spec (Octoscript-OH)

`weather.octoscript` is **LLM-generated from this spec**, not hand-authored. To
regenerate or restyle it, hand this document (plus the framework references
below) to an LLM and have it write `crates/octoscript-oh-webview/assets/weather.octoscript`.
...
- `assets/catalog.octoscript` — a large example of DSL style (helpers, loops, node literals).
