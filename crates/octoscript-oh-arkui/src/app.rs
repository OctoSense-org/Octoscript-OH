...
extern "C" {
    fn octoscript_content_remove(content: NodeContentHandle, root: crate::arkui::NodeHandle) -> c_int;
...
extern "C" fn on_event(target_id: i32, _event_type: i32) {
    ...
    if WECHAT_ACTIVE.with(|w| *w.borrow()) {
        ...
        // an app is. The router lives in octoscript-oh-webview -- it dispatches to
...
pub fn set_root(new_root: Option<Node>) {
    ...
    APP.with(|a| {
        ...
        if let Some(old) = app.root.take() {
            unsafe { octoscript_content_remove(slot, old.raw()) };
            drop(old);
        }
...
pub fn rebuild() {
    ...
    // Mount the flutter/samples kit — the same `.octoscript` the makepad backend
