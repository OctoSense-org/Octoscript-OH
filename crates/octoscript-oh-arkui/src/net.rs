//! Network capability for the Octoscript VM.
//!
...
extern "C" {
    ...
    /// and must be released with `octoscript_free`.
    fn octoscript_http_get(url: *const c_char, out_buf: *mut *mut c_char, out_len: *mut c_int)
        -> c_int;
    fn octoscript_free(p: *mut c_char);
}
...
pub fn http_get(url: &str) -> (i32, Option<Vec<u8>>) {
    ...
    let mut len: c_int = 0;
    let code = unsafe { octoscript_http_get(c.as_ptr(), &mut buf, &mut len) };
...
//
// The 2026-07-30 portability test lowered ONE semantic plan to both makepad Octoscript
...
// octos-one/aichat/widgets/src/octoscript.rs — same semantics, same reasoning — so a
