//! Octoscript DSL → native ArkUI widgets.
//!
//! This is the piece that makes the repo's claim true: the widget tree is not
//! written in Rust, it is **evaluated by the Octoscript VM** (`makepad-script`) at
...
//! catalog leans on that heavily (see `assets/catalog.octoscript`).

use crate::arkui::{attr, event, ty, Node};
// The VM comes from ymote/Octoscript, which re-exports it as `vm` from its own
// pinned vendored copy. Importing it under the old name keeps the walk below
// unchanged; what changes is provenance, not API.
use octoscript_core::vm as makepad_script;
...
pub fn build_screen(screen: &str, bench: Option<&str>) -> Option<Node> {
    const CATALOG: &str = include_str!("../../../examples/material-catalog/assets/catalog.octoscript");
    let esc = |s: &str| {
        s.replace('\\', "\\\\")
            .replace('"', "\\\"")
            .replace('\n', "\\n")
...
/// The component ids `catalog.octoscript` lists, in the order it lists them.
///
...
/// width `catalog.octoscript` renders it. Matches the DSL's own 2.2 chars/vp rule.
...
///
/// Same `.octoscript` source the makepad backend renders — vendored from
/// Octoscript-Makepad's `components/flutter/` by its `assemble` example. That the
...
pub fn build_flutter(route: &str, dark: bool) -> Option<Node> {
    const KIT: &str = include_str!("../../../examples/flutter-catalog/assets/flutter.octoscript");
...
/// native ArkUI node tree, so Octoscript-OH renders the app shell and the Web sits
/// below it.
pub fn build_youtube() -> Option<Node> {
    const YOUTUBE: &str = include_str!("../assets/youtube.octoscript");
    build(YOUTUBE)
}
...
pub fn build(src: &str) -> Option<Node> {
    ...
    let value = vm.eval(ScriptMod {
        ...
        module_path: String::from("octoscript_oh"),
        file: String::from("catalog.octoscript"),
        line: 0,
        column: 0,
        code: src.to_string(),
        values: Vec::new(),
    ...
    if value.is_nil() {
        ...
        // legitimately returned nothing. octoscript-core can tell them apart, so ask.
        match octoscript_core::check_syntax(src) {
...
/// these for web pages (`octoscript.invoke`); this hands the same registry to the
/// DSL.
///
