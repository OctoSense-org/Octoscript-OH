...
//!
//! Deliberately a second copy of `octoscript-render/src/state.rs`, not a shared
//! dependency. The two backends do not share a VM: this crate walks the kit on
//! octoscript-core's, the makepad backend on octoscript-render's, and octoscript-oh-arkui
//! has no path to octoscript-render at all. Making one appear would mean a
