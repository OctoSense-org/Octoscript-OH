...
//!
//! `octoscript-oh` is a `cdylib`. A `cdylib` is a final artifact: nothing links
...
//!
//! ```text
//! octoscript-oh-plugin-demo  (rlib)  ─┐
//!                                 ├─> octoscript-oh-core (rlib)
//! octoscript-oh              (cdylib) ┘        the registry
...
fn send(slot: u32, call_id: String, result: Result<String, String>) {
    ...
    match f {
        ...
        None => eprintln!("octoscript-oh-core: no reply channel; dropped answer for {call_id}"),
    }
}
...
/// the app serves it at `/__octoscript.js`, and a developer running a Vite dev
/// server needs the same bytes on disk. `octoscript-oh-cli shim` writes them.
pub const SHIM_JS: &str = r#"(function(){
  ...
  window.octoscript = {
    ...
    invoke: function (tool, args) {
      return new Promise(function (res, rej) {
        ...
        if (!window.octoscript_native || !window.octoscript_native.invoke) {
          delete pending[id];
          rej(new Error('octoscript_native bridge not present'));
          return;
        }
        ...
        window.octoscript_native.invoke(id, tool,
    ...
    available: function () {
      return !!(window.octoscript_native && window.octoscript_native.invoke);
    }
  };
  ...
  if (window.octoscript_native && window.octoscript_native.invoke) {
    window.octoscript_native.invoke('ready', 'slot.ready', '');
  }
})();
"#;
