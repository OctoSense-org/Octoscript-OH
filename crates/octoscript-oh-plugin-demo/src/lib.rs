...
//!
//! This crate does not depend on `octoscript-oh`, cannot see `bridge.rs`, and knows
//! nothing about napi, ArkTS, web slots or the capability gate. It depends only
//! on `octoscript-oh-core`, which is the whole point: if a tool defined here is
...
/// see the note on `REGISTRY` in `octoscript-oh-core`.
