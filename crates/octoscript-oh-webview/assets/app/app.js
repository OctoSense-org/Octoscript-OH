...
//    the shim arrived as a served file, but octoscript_native is injected by ArkTS
//    and only for slots Rust marks trusted.
if (!window.octoscript || !window.octoscript.invoke) {
  mark('r-bridge', false, 'no shim');
} else {
  window.octoscript.invoke('device.info', {})
...
function permCheck(id, names, wantRefused) {
  if (!window.octoscript || !window.octoscript.invoke) { mark(id, false, 'no shim'); return; }
  window.octoscript.invoke('permission.request', names)
    .then(function (r) {
...
//
//        octoscript-oh-plugin-demo depends on octoscript-oh-core and nothing else. It
