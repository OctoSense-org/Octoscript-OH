...
//!
//! ```text
//! WebviewController.customizeSchemes([{schemeName: 'octoscript', ...}])  register
//! controller.setWebSchemeHandler('octoscript', handler)                  install
...
//! `OCTOSCRIPT_FRONTEND_DIR`, or `assets/app` — and embeds everything under it,
...
//! goes through a dev server; see `OCTOSCRIPT_DEV_SERVER`.
//!
//! # The shim
//!
...
//! `/__octoscript.js` and the page includes it like any other script. That is a
...
pub const SCHEME: &str = "octoscript";
...
/// `OCTOSCRIPT_FRONTEND_DIR` to a `dist/` and whatever is in it ships as-is.
include!(concat!(env!("OUT_DIR"), "/bundle.rs"));

/// The dev server this build points at, if any.
///
/// Set `OCTOSCRIPT_DEV_SERVER=http://192.168.1.20:5173` at build time and an app
/// slot navigates there instead of to `octoscript://app`. The frontend then comes
...
/// frontend at all.
///
/// Not a runtime switch. A release build with no `OCTOSCRIPT_DEV_SERVER` has no
...
pub fn dev_server() -> Option<&'static str> {
    match option_env!("OCTOSCRIPT_DEV_SERVER") {
...
///
/// `octoscript://app` normally; the dev server's origin when this build has one.
...
/// `octoscript://app/a/b?q=1#frag` -> `/a/b`.
///
...
pub fn self_test() {
    let cases: &[(&str, &str)] = &[
        ...
        ("octoscript://app", "/index.html"),
        ("octoscript://app/app.js", "/app.js"),
        ("octoscript://app/app.js?v=2", "/app.js"),
        ("octoscript://app/a/b.css#x", "/a/b.css"),
        // Traversal is refused, not resolved.
        ("octoscript://app/../secret", "/index.html"),
        ("octoscript://app/a/../../etc/passwd", "/index.html"),
    ];
    let mut bad = 0;
    ...
    for (input, want) in [
        ...
        ("octoscript://app/index.html", "octoscript://app"),
    ] {
...
mod tests {
    ...
    #[test]
    fn paths_normalise() {
        assert_eq!(normalise("octoscript://app/"), "/index.html");
        assert_eq!(normalise("octoscript://app"), "/index.html");
        assert_eq!(normalise("octoscript://app/app.js"), "/app.js");
        assert_eq!(normalise("octoscript://app/app.js?v=2"), "/app.js");
        assert_eq!(normalise("octoscript://app/a/b.css#x"), "/a/b.css");
    }

    #[test]
    fn traversal_is_refused_not_resolved() {
        assert_eq!(normalise("octoscript://app/../secret"), "/index.html");
        assert_eq!(normalise("octoscript://app/a/../../etc/passwd"), "/index.html");
    }
}
