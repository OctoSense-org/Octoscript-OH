...
//! `octoscript-oh-arkui` for the widget tree those pages sit in.
//!
//! # Why the split runs this way
//!
//! The dependency is one-directional and that is the whole reason it was worth
//! separating. Nothing in `octoscript-oh-arkui` mentions the bridge, a web slot,
...
//!
//! So: `octoscript-oh-arkui` renders; this crate exposes the phone to a page.
//! One `.so` still comes out, because ArkTS loads exactly one.

pub use octoscript_oh_arkui::{app, arkui, bench, dsl, log, mem, net};
...
use octoscript_oh_arkui::arkui::NodeContentHandle;
...
pub fn mount(env: Env, content: JsObject) -> napi_ohos::Result<()> {
    ...
    // octoscript-oh-arkui has no idea what an app is, and wiring this here is what
    ...
    if status != 0 || slot.is_null() {
        log("octoscript-oh: could not resolve NodeContent from the napi value");
        return Ok(());
    }
    ...
    // it is not part of. See octoscript-oh-core's module docs.
    // Before any plugin can answer, it needs somewhere to answer to.
    octoscript_oh_core::set_reply(bridge::plugin_reply);
    octoscript_oh_core::with_registry_mut(|r| {
        octoscript_oh_plugin_demo::register(r);
    });
    log(&format!(
        "plugins: {} tool(s) registered: {:?}",
        octoscript_oh_core::registered().len(),
        octoscript_oh_core::registered()
    ));
    log(&octoscript_oh_core::self_test());
    log(&caps::self_test());
    assets::self_test();
    app::init(slot);
    ...
    // pages. This exposes the same registry to the Octoscript DSL.
    ...
    octoscript_oh_arkui::set_web_reset(webslot::reset);
    octoscript_oh_arkui::set_web_declare(|src, x, y, w, h| {
    ...
    octoscript_oh_arkui::set_web_declare_html(webslot::declare_html);
    octoscript_oh_arkui::set_host_invoke(|tool| match tool {
        ...
        "sensor.steps" => sensor::sample(sensor::type_from_name("pedometer").unwrap_or(266), 400)
            .unwrap_or_else(|e| e),
        // asset_transformation: the octoscript:// protocol resolves a request URL
        // to bytes at serve time, which is this stack's asset pipeline.
        "assets.shim" => {
            let a = assets::get("octoscript://app/__octoscript.js");
            format!(
                "{{\"path\":\"/__octoscript.js\",\"mime\":\"{}\",\"status\":{},\"bytes\":{}}}",
                a.mime,
                a.status,
                a.body.len()
            )
        ...
        "assets.index" => {
            let a = assets::get("octoscript://app/index.html");
            format!(
        ...
        "assets.missing" => {
            let a = assets::get("octoscript://app/nope.bin");
            format!(
        ...
        "embed.nodes" => format!("{}", octoscript_oh_arkui::ui::last_total()),
...
pub fn wechat_render() -> Vec<f64> {
    ...
    // the Octoscript DSL in assets/weather.octoscript) instead of the WeChat benchmark.
    let node = octoscript_oh_arkui::dsl::build_weather();
    app::set_root(node);
    vec![0.0, 0.0]
}
...
/// Octoscript tree can contain a webview -- see `webslot.rs`.
/// One asset from the shipped bundle, for the `octoscript://` scheme handler.
///
...
/// Evaluate a tiny script through the Octoscript VM and report what came back.
///
...
/// `.octoscript` card, as a blank screen. Worth one call to rule out.
...
/// A page called `octoscript.invoke(tool, args)`. Fire-and-forget; the answer
...
/// its page with `octoscript._resolve`.
...
pub fn app_take_dirty() -> u32 {
    ...
    if apps::weather_web::take_dirty() | location::take_dirty() | wonderous::met::take_dirty() {
        1
    } else {
        0
    }
...
/// cannot live in the native ArkUI node tree, so Octoscript-OH renders the app
/// shell and the Web is laid out beneath it.
#[napi(js_name = "youtubeRender")]
pub fn youtube_render() {
    let node = octoscript_oh_arkui::dsl::build_youtube();
    app::set_root(node);
}
...
pub fn wonders_bench() {
    ...
    runs.sort_unstable();
    octoscript_oh_arkui::log(&format!(
    ...
    for (i, name) in names.iter().enumerate() {
        let before = octoscript_oh_arkui::ui::count();
        let t0 = std::time::Instant::now();
        let node = match i {
            0 => wonders::screens::intro(0, W, H),
            1 => wonders::home::build(0, W, H),
        ...
        let nodes = octoscript_oh_arkui::ui::count().saturating_sub(before);
        total_us += us;
        total_nodes += nodes;
        octoscript_oh_arkui::log(&format!(
...
pub fn app_data(key: String) -> Vec<String> {
    use wonderous_makepad as wonderous;
    match key.as_str() {
...
pub fn go_back() -> bool {
    ...
    let node = octoscript_oh_arkui::dsl::build_flutter(&parent, false);
    app::set_root(node);
    true
}
...
pub fn catalog_screen(name: String) -> u32 {
    ...
    let node = octoscript_oh_arkui::dsl::build_flutter(route, false);
    let n = octoscript_oh_arkui::ui::count();
    octoscript_oh_arkui::ui::record_total(n);
    app::set_root(node);
    n as u32
}
