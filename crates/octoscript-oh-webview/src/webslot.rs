...
//! a webview in a Octoscript tree has to be an **ArkTS `Web` component positioned
...
pub enum Source {
    ...
    Html(String),
    /// A page from the bundle this app ships, served over the `octoscript://`
...
/// the same `octoscript_native` object the weather card has, with `octoscript.eval` and
/// `http.get` on it.
impl Source {
...
pub fn html_for(id: u32) -> String {
    slots()
        ...
        .and_then(|s| match &s.source {
            ...
            // slot navigates to a octoscript:// URL the scheme handler answers.
...
use octoscript_oh_arkui::ui::col;
...
/// `octoscript.invoke(tool, args)` without having to carry the plumbing itself.
