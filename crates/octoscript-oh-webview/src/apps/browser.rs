//! A browser card: native Octoscript chrome around an ArkTS `Web` surface.
//!
//! This exists to exercise `webslot` rather than to be a browser. The point it
//! proves is that a Octoscript tree can contain a web view *at a position the DSL
...
pub const TABS: &[(&str, &str)] = &[
    ...
    // reach octoscript_native -- turning "verified by construction" into
    // "verified on device".
