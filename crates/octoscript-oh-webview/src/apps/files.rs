...
(function () {{
  ...
  function probe() {{
    ...
    paths.forEach(function (p) {{
      ...
      octoscript.invoke('fs.list', p).then(function (r) {{
  ...
  function pick(mode) {{
    ...
    octoscript.invoke('fs.pick', {{ mode: mode }}).then(function (sel) {{
      if (!sel || !sel.length) {{
      ...
      octoscript.invoke('fs.stat', s.path).then(function (st) {{
        ...
        listEl.innerHTML = head + '<div class=hint>reading&#8230;</div>';
        octoscript.invoke('fs.list', target).then(function (r) {{
          render(r, head);
        }}).catch(function (e) {{
