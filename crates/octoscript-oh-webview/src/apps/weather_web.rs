//! A weather app rendered as a web page inside a Octoscript tree.
//!
//! The companion to `assets/weather.octoscript`, which draws the same data with
...
fn page_with(fc: Forecast, city: usize, w: f32, h: f32) -> String {
    ...
    // What the Octoscript VM computes over. It used to be a hardcoded literal,
...
<div class=tile style="margin-top:12px">
 ...
 <div class=br><span class=bk>octoscript.eval</span><span class="bv" id=vm>&#8230;</span></div>
...
(function () {{
  ...
  octoscript.invoke('device.info').then(function (i) {{
    el.textContent = i.platform + ' \u00b7 slot ' + i.slot;
  }}).catch(function (e) {{ el.textContent = 'failed: ' + e.message; }});

  // The page runs Octoscript. `input` is JSON this page supplied; the script
  ...
  octoscript.invoke('octoscript.eval', {{
    // Canonical Octoscript wants one statement per line -- the one-liner form
    // of this was rejected at line 2 col 43, which is exactly the located
    // error octoscript-core gives that the bare VM did not.
    source: [
      'let hi = 0',
  ...
  // the whole chain ran: page JS -> octoscript_native -> Rust -> network -> reply
  ...
  octoscript.invoke('echo', 'ping-{city}')
    .then(function (r) {{
  ...
  octoscript.invoke('log', 'card ready: {name}');
  ...
  Promise.all(probes.map(function (p) {{
    return octoscript.invoke('http.get', p[1])
...
/// twin of this card lives in `assets/weather.octoscript`.
#[allow(dead_code)]
const _NATIVE_TWIN: (&str, u32) = ("assets/weather.octoscript", CREAM);
