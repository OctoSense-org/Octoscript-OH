...
pub fn page() -> (f32, f32) {
    ...
    if ok && pw > 0 && ph > 0 && ratio > 0.1 {
        ...
        (octoscript_oh_arkui::ui::W, octoscript_oh_arkui::ui::PAGE_H)
    }
}
...
fn measure_root(root: usize) -> bool {
    ...
    let Some((w_px, h_px)) = (unsafe {
        octoscript_oh_arkui::arkui::layout_size(root as octoscript_oh_arkui::arkui::NodeHandle)
    }) else {
        return false;
    };
    let r = ratio();
...
fn fade_in_screen() {
    ...
    let n = root as octoscript_oh_arkui::arkui::NodeHandle;
    unsafe {
        octoscript_oh_arkui::arkui::animate(
            ...
            move || unsafe {
                Node::set_f32_attr_raw(n, octoscript_oh_arkui::arkui::attr::opacity(), 1.0)
            },
        )
    };
}
...
pub fn build() -> Option<Node> {
    ...
    // is wanted.
    let node = node
        .f32_attr(octoscript_oh_arkui::arkui::attr::opacity(), 0.0)
        .on_event(octoscript_oh_arkui::arkui::event::appear(), SCREEN_APPEAR);
