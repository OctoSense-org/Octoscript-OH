...
pub enum App {
    ...
    Browser,
    /// The LLM-generated weather card (`assets/weather.octoscript`): DSL evaluated
    ...
    ///
    /// Distinct from `Catalog`, which walks the vendored `.octoscript` kit. Both
    ...
    Flutter,
    /// The Material catalog, rendered entirely by the Octoscript VM.
    ///
    /// The one app here with no Rust building its tree: `catalog.octoscript` is
    /// 482 lines of DSL evaluated on device, and every widget it produces is a
    /// real ArkUI node. No makepad anywhere in the path.
    Catalog,
    /// The shipped frontend bundle, served over `octoscript://` by `assets.rs`.
    ///
    ...
    /// `assets/weather.octoscript`, which draws the same Open-Meteo data with real
        ...
        match nav.app {
            ...
            App::Flutter => {
                if let Some(action) = flutter_catalog::action_for(target) {
                    return octoscript_oh_arkui::state::apply(&action);
                }
                if let Some(route) = flutter_catalog::route_for(target) {
                    octoscript_oh_arkui::app::set_screen_quiet(route);
                    return true;
                }
                if target == flutter_catalog::BACK {
                    let cur = octoscript_oh_arkui::app::current_screen();
                    ...
                    octoscript_oh_arkui::app::set_screen_quiet(parent);
                    return true;
                }
                false
            }
            ...
            App::Catalog => match target {
                octoscript_oh_arkui::dsl::CATALOG_NAV_BACK => {
                    let was = nav.sub != 0;
                    nav.sub = 0;
                    was
                }
                t if t >= octoscript_oh_arkui::dsl::CATALOG_NAV_BASE => {
                    let idx = (t - octoscript_oh_arkui::dsl::CATALOG_NAV_BASE) as usize;
...
pub fn build() -> (Option<Node>, usize, f64) {
    ...
    octoscript_oh_arkui::net::mark_ui_thread();
    ...
    octoscript_oh_arkui::ui::reset_count();
    crate::webslot::reset();
    let t0 = Instant::now();
    let node = match app {
        ...
        App::Weather => octoscript_oh_arkui::dsl::build_weather(),
        App::PlanWeather => octoscript_oh_arkui::dsl::build_planweather(),
        App::Flutter => {
            let route = octoscript_oh_arkui::app::current_screen();
        ...
        App::Catalog => {
            ...
            // `.octoscript` the makepad backend renders, walked into ArkUI. Taps
            ...
            octoscript_oh_arkui::dsl::build_flutter(&route, false)
        }
        App::WeChat => unreachable!(),
    };
    let us = t0.elapsed().as_nanos() as f64 / 1000.0;
    (node, octoscript_oh_arkui::ui::count(), us)
}
...
pub fn build_route(app: App, tab: usize, route: &str) -> (usize, f64) {
    ...
    let node = match app {
        ...
        App::Catalog => {
            ...
            octoscript_oh_arkui::dsl::build_flutter(screen, false)
        }
        App::WeChat => unreachable!(),
    };
    let us = t0.elapsed().as_nanos() as f64 / 1000.0;
