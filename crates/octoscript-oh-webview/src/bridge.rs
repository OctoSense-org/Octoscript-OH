...
//! Octoscript web slot calls:
//!
//! ```js
//! octoscript.invoke('device.info').then(info => …)
//! ```
//!
...
//!
//! 1. JS calls `octoscript_native.invoke(callId, tool, argsJson)` and gets nothing
...
//!    `octoscript._resolve(callId, payload)` back into the page, settling it.
//!
...
use octoscript_oh_arkui::net;
use std::collections::VecDeque;
use std::sync::Mutex;

/// Caps for anything a page hands to the VM.
///
/// A page is the least trusted thing in the process. `octoscript-core` is built to
...
fn deliver(slot: u32, id: &str, payload: &str) -> bool {
    let js = if let Some(event) = id.strip_prefix('@') {
        format!(
            "window.octoscript&&window.octoscript._event&&window.octoscript._event({},{payload})",
            json_str(event)
        )
    } else {
        format!(
            "window.octoscript&&window.octoscript._resolve&&window.octoscript._resolve({},{payload})",
            json_str(id)
        )
    };
...
/// The reply channel handed to `octoscript-oh-core` at startup.
///
...
/// ```
/// Moved to `octoscript-oh-core` so a plugin crate can take one without depending
/// on this cdylib. Re-exported because every tool arm below uses the name.
pub use octoscript_oh_core::Args;
...
pub fn invoke(slot: u32, call_id: String, tool: String, args_raw: String) {
    ...
    if octoscript_oh_core::claims(&tool) {
        octoscript_oh_core::dispatch(
            &tool,
            &args,
            octoscript_oh_core::Responder::new(slot, call_id.clone()),
        );
        return;
    }
    ...
    match tool.as_str() {
        ...
        "plugin.list" => {
            let names = octoscript_oh_core::registered();
        ...
        // The Octoscript half of the bridge: a page evaluates DSL and gets JSON
        // back. This is what makes it a JS <-> Rust/Octoscript bridge rather than
        ...
        //
        // Args: {"source": "<octoscript>", "input": <any json>}
        ...
        "octoscript.eval" => {
            std::thread::spawn(move || {
                ...
                reply(slot, call_id, eval_octoscript(source, parsed.get("input")));
            });
        }
...
fn page_limits() -> octoscript_core::ExecutionLimits {
    let d = octoscript_core::ExecutionLimits::default();
    octoscript_core::ExecutionLimits {
...
fn eval_octoscript(source: &str, input: Option<&serde_json::Value>) -> String {
    let mut rt = match octoscript_core::Runtime::<(), ()>::with_limits((), (), page_limits()) {
        Ok(r) => r,
    ...
    let evaluation = match rt.eval(source) {
        ...
        Err(e) => {
            ...
            crate::log(&format!("octoscript.eval stopped: {m}"));
            return err(&m);
        }
    };
    ...
    match rt.script_value_as_json(evaluation.value, JSON_MAX_BYTES, JSON_MAX_DEPTH) {
        ...
        Err(e) => {
            crate::log(&format!("octoscript.eval result not encodable: {e:?}"));
...
///
/// `octoscript_native` is the raw proxy ArkWeb injects: synchronous and void. This
/// wraps it in the promise API a page actually wants, and provides `_resolve`
/// for ArkTS to call back into.
/// The shim now lives in `octoscript-oh-core`, so a host-side tool can print it
/// without building this cdylib -- a dev server has to serve it too.
pub use octoscript_oh_core::SHIM_JS;
