...
```
my-app/
  octoscript.toml        名称、bundle id、版本、图标、权限
  index.html         必须加载 /__octoscript.js，见下
  src/main.js        你的前端
  src/style.css
  public/__octoscript.js 桥接 shim，自动生成
  plugin/            你自己的原生代码，Rust
  build.sh           依托一份 Octoscript-OH 源码来构建
  README.md
```
...
没有它就没有 `window.octoscript`。发布构建里这个 URL 由应用自己响应；开发服务器不
知道有这么个东西，所以 `octoscript-oh new` 会在 `public/` 里写一份，`octoscript-oh shim`
负责更新它。
...
`build.sh` 会去找一份 Octoscript-OH 源码——`OCTOSCRIPT_OH`，或者同级目录——然后带上指向
你 `dist/` 的 `OCTOSCRIPT_FRONTEND_DIR` 交给它去构建。那边负责编译 Rust、放置
`.so`、跑 hvigor、安装并启动。
...
```sh
octoscript-oh dev                                       # 每次开工执行一次
npm run dev                                         # 终端 1
OCTOSCRIPT_DEV_SERVER=http://127.0.0.1:5173 ./build.sh  # 终端 2
```

此后应用从打包工具那里加载前端，而不是从内嵌的资源，所以改动会直接在设备上刷新，
不用重新构建。改 Rust 仍然要。

`octoscript-oh dev` 用 `hdc rport` 打通一条 USB 隧道，把手机的 `127.0.0.1:5173` 映射
...
隧道在重新插拔或手机重启后就没了。页面加载不出来时重跑一次 `octoscript-oh dev`。

`OCTOSCRIPT_DEV_SERVER` 在构建时读取，不是运行时。一个能在已发布应用里被打开的调试
...
密码永远不写进这个文件。签名从环境变量 `OCTOSCRIPT_SIGN_PWD` 读。
...
把它链接进去目前需要在 Octoscript-OH 那份源码里改两处，因为 `.so` 是在那边构建的，
只有产出它的那个 crate 才能把插件拉进二进制。在你动手之前，模板里的例子会显示
`not linked yet — see README`。这部分是还没自动化的。

## 排查

**白屏。** 在 hilog 里搜 `OCTOSCRIPTASSET`：每个提供出去的文件都会带状态码和大小
打一行。404 那行会写出被请求的路径。

**什么都没提供。** 找 `OCTOSCRIPTSCHEME registered octoscript://` 和
